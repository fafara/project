 ...
 */
public class BorrowRecordFragment extends Fragment implements IBorrowRecordCallBack {
    ...
    private void initData() {
        mListener.requestBorrowRecord(status, "borrow_record");
    }
    ...
    public void recordCallBackSuccess(List<BorrowRecordsResponse.ResultBean> list) {
        ...
        if (list.isEmpty()) {
            mNoDataList.setVisibility(View.VISIBLE);
            mTips.setText("暂无借款记录");
            CLogUtil.showToast(getActivity(), "暂无借款记录");
            return;
        }
