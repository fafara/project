 ...
 * 完美点券
 */
@EActivity(R.layout.activity_wm_pay)
public class WanMeiActivity extends BaseActivity {

    @ViewById(R.id.gv_wmpay)
    GridView mWMPayGridView;
    ...
    private String gameid = "GAME52182";
    ...
    public void initViews() {
        setTitleLayout("完美点券", true);
        setRightImgMessage("用户须知", RyxAppconfig.Notes_WMRecharge);
        initQtPatParams();
        doOrderRequest();
    }
    ...
    private void doOrderRequest() {
        ...
        httpsPost("doAliPayReq", new XmlCallback() {
            ...
            public void onTradeSuccess(RyxPayResult payResult) {
                ...
                LogUtil.showLog("muxin", "wm json:" + jsonStr);
                initListData();
                initData();
            }
        });
    ...
    private boolean checkInput() {
        mobile = mWMPayEt.getText().toString().trim();
        if (TextUtils.isEmpty(mobile)) {
            LogUtil.showToast(WanMeiActivity.this, getResources().getString(R.string.please_enter_account));
            return false;
        } else if (TextUtils.isEmpty(mMoney)) {
            LogUtil.showToast(
                    WanMeiActivity.this, "请选择充值金额!!!");
            return false;
        }
        return true;
    }
    ...
    public void rechargeClick() {
        if (checkInput()) {
            Intent intent = new Intent(WanMeiActivity.this, CreateOrder_.class);
            orderinfo = Order.WM_RECHARGE;
    ...
    private void initData() {
        adapter = QqCoinAccountAdapter_.getInstance_(WanMeiActivity.this);
        adapter.setList(coinlist);
        adapter.setSelection(0);
        mMoney = MoneyEncoder.EncodeFormat(coinlist.get(0).getParvalue());
        mWMPayGridView.setAdapter(adapter);
    }
    ...
    private void initListData() {
        try {
            ...
            for (int i = 0; i < banks.length(); i++) {
                ...
                qcoinInfo.setTipName("完美点券" + parvalue);
