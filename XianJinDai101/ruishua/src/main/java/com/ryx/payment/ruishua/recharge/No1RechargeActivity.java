...
@EActivity(R.layout.activity_no1recharge)
public class No1RechargeActivity extends BaseActivity {

    @ViewById(R.id.et_mobile)
    EditText mMobileEt;
    @ViewById(R.id.layout_mobile)
    LinearLayout mMobileLine;
    @ViewById(R.id.gv_no1)
    GridView mNo1GridView;
    private String mMoney;
    private Param qtpayGameId;
    private String gameid = "CARD60320";
    ...
    public void initViews() {
        setTitleLayout("1号店礼品卡", true);
        setRightImgMessage("用户须知", RyxAppconfig.Notes_NO1Recharge);
        initQtPatParams();
        doOrderRequest();
    }
    ...
    private void doOrderRequest() {
        ...
        httpsPost("doAliPayReq", new XmlCallback() {
            ...
            public void onTradeSuccess(RyxPayResult payResult) {
                ...
                LogUtil.showLog("muxin", "no1 json:" + jsonStr);
                initListData();
                initData();
            }
        });
    ...
    private void initData() {
        mMobileEt.setText(QtpayAppData.getInstance(No1RechargeActivity.this).getMobileNo());
        mMobileEt.setSelection(mMobileEt.getText().length());
        adapter = MobileAdapter_.getInstance_(No1RechargeActivity.this);
        adapter.setList(coinlist);
        adapter.setSelection(0);
        mMoney = MoneyEncoder.EncodeFormat(coinlist.get(0).getParvalue());
        mNo1GridView.setAdapter(adapter);
    }

    @ItemClick(R.id.gv_no1)
    ...
    private boolean checkInput() {
        ...
        if (TextUtils.isEmpty(mobile) || mobile.length() != 11) {
            LogUtil.showToast(No1RechargeActivity.this, getResources().getString(R.string.please_enter_the_11_phone_numbers));
            return false;
        } else if (mobile.substring(0, 1).equals("0")) {
            LogUtil.showToast(No1RechargeActivity.this, getResources().getString(R.string.not_a_phone_number));
            return false;
        } else if (TextUtils.isEmpty(mMoney)) {
            LogUtil.showToast(
                    No1RechargeActivity.this, "请选择充值金额!!!");
            return false;
        }
        return true;
    }
    ...
    public void rechargeClick(){
        if (checkInput()) {
            Intent intent = new Intent(No1RechargeActivity.this, CreateOrder_.class);
            orderinfo = Order.NO1_RECHARGE;
