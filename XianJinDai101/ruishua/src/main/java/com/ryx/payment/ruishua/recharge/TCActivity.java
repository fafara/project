 ...
 * 世纪天成
 */
@EActivity(R.layout.activity_tc_pay)
public class TCActivity extends BaseActivity {

    @ViewById(R.id.gv_tcpay)
    GridView mTCPayGridView;
    ...
    private String gameid = "GAME1983";
    ...
    public void initViews() {
        setTitleLayout("世纪天成", true);
        setRightImgMessage("用户须知", RyxAppconfig.Notes_SJTCRecharge);
        initQtPatParams();
        doOrderRequest();
    }
    ...
    private void doOrderRequest() {
        ...
        httpsPost("doAliPayReq", new XmlCallback() {
            ...
            public void onTradeSuccess(RyxPayResult payResult) {
                ...
                LogUtil.showLog("muxin","tc json:"+jsonStr);
                initListData();
                initData();
            }
        });
    ...
    private boolean checkInput() {
        mobile = mTCPayEt.getText().toString().trim();
        if (TextUtils.isEmpty(mobile)) {
            LogUtil.showToast(TCActivity.this, getResources().getString(R.string.please_enter_account));
            return false;
        }else if (TextUtils.isEmpty(mMoney)) {
            LogUtil.showToast(
                    TCActivity.this, "请选择充值金额!!!");
            return false;
        }
        return true;
    }
    ...
    public void rechargeClick() {
        if (checkInput()) {
            Intent intent = new Intent(TCActivity.this, CreateOrder_.class);
            orderinfo = Order.SJTC_RECHARGE;
    ...
    private void initData() {
        adapter = QqCoinAccountAdapter_.getInstance_(TCActivity.this);
        adapter.setList(coinlist);
        adapter.setSelection(0);
        mMoney = MoneyEncoder.EncodeFormat(coinlist.get(0).getParvalue());
        mTCPayGridView.setAdapter(adapter);
    }
