...
public class XJDRepaymentAcitivity extends BaseActivity implements KeyboardChangeListener.KeyBoardListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.c_activity_xjd_quick_repayment);
    ...
    private TextView tv_current_remaining_repay_amount;
    ...
    private void initView() {
        ...
        tv_repay_amount.setText(CMoneyEncoder.EncodeFormat(String.valueOf(detailResponse.getTotal_amount())));
        //还款日
        ...
        //当前应还金额，包括手续费
        if (CNummberUtil.parseDouble(String.valueOf(detailResponse.getTerm_repay_amount()), 0.00d) <= 0) {
            c_tv_repayment_money.setText(CMoneyEncoder.EncodeFormat("0"));
        } else {
            c_tv_repayment_money.setText(CMoneyEncoder.EncodeFormat(String.valueOf(detailResponse.getTerm_repay_amount())));
        }
        tv_current_repay_amount = (TextView) findViewById(R.id.tv_current_repay_amount);
        tv_current_remaining_repay_amount = (TextView) findViewById(R.id.tv_current_remaining_repay_amount);
...
//        tv_current_repay_amount.setText(CMoneyEncoder.EncodeFormat(String.valueOf(detailResponse.getRemain_total_amount())));
        ...
        CBanksUtils.selectIcoidToImgView(XJDRepaymentAcitivity.this, repaymentBankCode, c_dfbank_iv);
    }
    ...
    private void setOverdueStatus(boolean isOverdue) {
        ...
        //当期应还金额
        if (!TextUtils.isEmpty(String.valueOf(detailResponse.getTerm_repay_amount()))) {
            setRemainSumAmount(String.valueOf(detailResponse.getTerm_repay_amount()));
        } else {
            setRemainSumAmount("0.00");
        }
        //剩余应还金额
    ...
    private void setSumAmount(String money) {
        ...
        tv_current_remaining_repay_amount.setText(Html.fromHtml(s));
    }
     ...
     * @param money
     */
    private void setRemainSumAmount(String money) {
        String m = String.format(getResources().getString(R.string.c_borrow_record_current_remaining_payment),
                CMoneyEncoder.EncodeFormat(money));
        tv_current_repay_amount.setText(Html.fromHtml(m));
    }
    ...
    NoDoubleClickListener clickListener = new NoDoubleClickListener() {
        ...
        protected void onNoDoubleClick(View view) {
            ...
            if (id == R.id.lay_repaystyle) {
                ...
                Intent intent = new Intent(XJDRepaymentAcitivity.this, BankSelectActivity.class);
    ...
    public void showHelpPopWindow() {
        ...
        tv_content.setText(getString(R.string.repayamount_periodamount));
//
        ...
        text_arriveamount.setText(getString(R.string.repayamount_periodtotal));
        TextView arriveTv = (TextView) view.findViewById(R.id.tv_arriveAmount);
        arriveTv.setText(String.valueOf(detailResponse.getSchedule_curr_bal()));
//        利息
/*        AutoLinearLayout lay_interest = (AutoLinearLayout) view.findViewById(R.id.lay_interest);
        ...
        if (!TextUtils.isEmpty(detailResponse.getInterest_amount())) {
            interestTvv.setText(String.valueOf(detailResponse.getInterest_amount()));
        } else {
...
/*        TextView serviceTv = (TextView) view.findViewById(R.id.tv_service_fee);
        if (!TextUtils.isEmpty(String.valueOf(detailResponse.getLoan_fee_sum()))) {
            serviceTv.setText(String.valueOf(detailResponse.getLoan_fee_sum()));
        } else {
...
/*        AutoLinearLayout lay_overdue_fee = (AutoLinearLayout) view.findViewById(R.id.lay_overdue_fee);
        ...
        if (!TextUtils.isEmpty(String.valueOf(detailResponse.getSub_overdue_interest_amount()))) {
            overdueTv.setText(String.valueOf(detailResponse.getSub_overdue_interest_amount()));
        } else {
        ...
        final com.rey.material.app.Dialog dialog = builder.build(XJDRepaymentAcitivity.this);
    ...
    private void change_dkCard() {
        ...
        httpsPost(this, request, ReqAction.APPLICATION_CHANGE_DKCARD, LoanRepayResponse.class, new ICallback<LoanRepayResponse>() {
            ...
            @Override
            public void failture(String tips) {
                CLogUtil.showToast(XJDRepaymentAcitivity.this, tips);
            }
        });
    }
    ...
    private void initViewSattus() {
        ...
        CBanksUtils.selectIcoidToImgView(XJDRepaymentAcitivity.this, hkBankCode, c_dfbank_iv);
    }
                    ...
                    public void onPositiveActionClicked(RyxSimpleConfirmDialog ryxSimpleConfirmDialog) {
                       /* c_payback_money_btn.setEnabled(false);
                        c_payback_money_btn.setBackgroundDrawable(ContextCompat.getDrawable(
                                XJDRepaymentAcitivity.this, R.drawable.roundshape_gray));*/
    ...
    private void gotoSwipe() {
        try {
            Intent intent = new Intent(XJDRepaymentAcitivity.this,
...
//            intentMapVal.put("cardIdx",cardIdx);//待确定
            ...
            c_payback_money_btn.setBackgroundDrawable(ContextCompat.getDrawable(
                    XJDRepaymentAcitivity.this, R.drawable.roundshape));
        } catch (Exception e) {
            CLogUtil.showToast(XJDRepaymentAcitivity.this, "数据错误,请稍后再试!");
        }
    }
        ...
        httpsPost(this, request, ReqAction.APPLICATION_LOAN_REPAY, LoanRepayResponse.class, new ICallback<LoanRepayResponse>() {
            ...
            public void success(LoanRepayResponse loanRepayResponse) {
                ...
                if (loanRepayCode==5031) {
                    ...
                    if (result == 1) {
                        CCommonDialog.showRepaymentOK(XJDRepaymentAcitivity.this, "还款成功", "如已结清，借款状态会在24:00之后变更", new CCommonDialog.IMessage() {
                            @Override
                        ...
                        CCommonDialog.showRepaymentTreat(XJDRepaymentAcitivity.this, "还款申请已受理 ", "最终还款结果请稍后查询", new CCommonDialog.IMessage() {
                            @Override
                            public void callback() {
                                finish();
                            }
                        ...
                        c_payback_money_btn.setEnabled(true);
                        CCommonDialog.showRepaymentError(XJDRepaymentAcitivity.this, "还款失败", "");
                    }
                }
            }
            ...
            public void failture(String tips) {
                ...
                c_payback_money_btn.setBackgroundDrawable(ContextCompat.getDrawable(
                        XJDRepaymentAcitivity.this, R.drawable.roundshape));*/
                CCommonDialog.showRepaymentError(XJDRepaymentAcitivity.this, "还款失败", tips);
            }
        });
    }
    ...
    public boolean checkInput() {
        if (!is_opened) {
            CLogUtil.showToast(XJDRepaymentAcitivity.this, "抱歉，非交易时间无法操作借款及还款！");
            return false;
        }
