...
@EActivity(R.layout.activity_amazonrecharge)
public class AmazonRechargeActivity extends BaseActivity {

    @ViewById(R.id.et_mobile)
    EditText mMobileEt;
    @ViewById(R.id.layout_mobile)
    LinearLayout mMobileLine;
    @ViewById(R.id.gv_amazon)
    GridView mAmazonGridView;
    private String mMoney;
    private Param qtpayGameId;
    private String gameid = "CARD60303";
    ...
    public void initViews() {
        setTitleLayout("亚马逊礼品卡", true);
        setRightImgMessage("用户须知", RyxAppconfig.Notes_YMXRecharge);
        initQtPatParams();
        doOrderRequest();
    }
    ...
    private void initData() {
        mMobileEt.setText(QtpayAppData.getInstance(AmazonRechargeActivity.this).getMobileNo());
        mMobileEt.setSelection(mMobileEt.getText().length());
        adapter = MobileAdapter_.getInstance_(AmazonRechargeActivity.this);
        adapter.setList(coinlist);
        adapter.setSelection(0);
        mMoney = MoneyEncoder.EncodeFormat(coinlist.get(0).getParvalue());
        mAmazonGridView.setAdapter(adapter);
    }

    @ItemClick(R.id.gv_amazon)
    ...
    private boolean checkInput() {
        ...
        if (TextUtils.isEmpty(mobile) || mobile.length() != 11) {
            LogUtil.showToast(AmazonRechargeActivity.this, getResources().getString(R.string.please_enter_the_11_phone_numbers));
            return false;
        } else if (mobile.substring(0, 1).equals("0")) {
            LogUtil.showToast(AmazonRechargeActivity.this, getResources().getString(R.string.not_a_phone_number));
            return false;
        } else if (TextUtils.isEmpty(mMoney)) {
            LogUtil.showToast(
                    AmazonRechargeActivity.this, "请选择充值金额!!!");
            return false;
        }
        return true;
    }
    ...
    public void rechargeClick(){
        if (checkInput()) {
            Intent intent = new Intent(AmazonRechargeActivity.this, CreateOrder_.class);
            orderinfo = Order.YMX_RECHARGE;
