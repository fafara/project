...
public class DeviceControllerImplForMe15 implements DeviceController{
	private DeviceLogger logger = DeviceLoggerFactory
			.getLogger(DeviceControllerImplForMe15.class);
	...
	private DeviceControllerImplForMe15() {
	}
	...
	public static DeviceController getInstance() {
		return new DeviceControllerImplForMe15();
	}
		...
		try {
			...
			case COMMON_SWIPER:
				...
				SwipResult swipRslt = getSwipResult(swiper,
						...
						TrackEncryptAlgorithm.BY_EPTOK_MODEL2);
...
//				ModuleType.COMMON_EMV);
		...
		if (deviceManager.getDevice().getDeviceInfo().getPID() == DeviceType.ME15B) {
			module = (EmvModule) deviceManager.getDevice().getExModule("EMV_LEVEL2");
		} else {
			module = (EmvModule) deviceManager.getDevice()
					.getStandardModule(ModuleType.COMMON_EMV);
		}
		module.initEmvModule(context);
		return module;
...
//		}
		...
		switch (openedModuleTypes[0]) {
		case COMMON_SWIPER: {
			Log.i("openedModuleTypes1", openedModuleTypes[0]+"---");
			CardResultType cardResultType = event.getCardResultType();
			//刷卡结果
			logger.info("Swipe results==" + cardResultType.toString()+" transLogNo==="+NewLandBlueToothAdapter.transLogNo);
			...
			SwipResult swipRslt = getTrackText(Const.CardType.COMMON,NewLandBlueToothAdapter.transLogNo);
			byte[] extraInfo=  swipRslt.getExtInfo();
			if (extraInfo==null) {
				Log.e(driverName,"extraInfo==null");
			}
//			Log.e(driverName,"extraInfo:"+ISOUtils.hexString(extraInfo));
...
//				}
		...
		case COMMON_ICCARD: {
			transferListener.onIcCardStartSwiper();
...
//			EmvTransInfo emvTransInfo = qPBOCModule.startQPBOC(amt, timeout, timeunit);
			EmvTransInfo emvTransInfo = qPBOCModule.startQPBOC(ProcessingCode.GOODS_AND_SERVICE, InnerProcessingCode.USING_STANDARD_PROCESSINGCODE, amt, timeout, timeunit);
//			transferListener.onQpbocFinished(emvTransInfo);
			SwipResult result =  getTrackText(Const.CardType.ICCARD, NewLandBlueToothAdapter.transLogNo);
//			byte[] extraInfo = result.getExtInfo();
//			if(extraInfo==null){
//				Log.e("devicename", "extraInfo==null");
//			}else{
				transferListener.onRFCardSwiped(result,emvTransInfo);
//			}
	...
	public SwipResult getTrackText(int flag,String flowid) throws InterruptedException {
		...
		SwipResult swipRslt = getSwipResult(swiper, trackKey, TrackEncryptAlgorithm.BY_RUIYINXIN_MODEL, flag,flowid);
		if (swipRslt.getRsltType() == SwipResultType.SUCCESS) {
			return swipRslt;
		}
		//交易撤销
	...
	private SwipResult getSwipResult(Swiper swiper, int trackKey, String encryptType, int flag,String flowid) {
		isConnected();
		SwipResult swipRslt;
		if (flag == Const.CardType.COMMON) {
			Log.e(driverName, "readEncryptResult1");
			swipRslt = swiper.readEncryptResult(new SwiperReadModel[] { SwiperReadModel.READ_SECOND_TRACK, SwiperReadModel.READ_THIRD_TRACK }, new WorkingKey(trackKey), encryptType, null, flowid);
//			swipRslt = swiper.readEncryptResult(new SwiperReadModel[] { SwiperReadModel.READ_SECOND_TRACK, SwiperReadModel.READ_THIRD_TRACK }, new WorkingKey(trackKey), encryptType);
			Log.e(driverName, "readEncryptResult2");
		} else {
			swipRslt = swiper.readEncryptResult(new SwiperReadModel[] { SwiperReadModel.READ_IC_SECOND_TRACK }, new WorkingKey(trackKey), encryptType, null, flowid);
		}
		return swipRslt;
	}
	@Override
	public PinInputEvent startPininput(String acctHash, int inputMaxLen,
		...
		PinInputEvent event = pinInput
				.startStandardPinInput(new WorkingKey(
						4),
						PinManageType.MKSK, AccountInputType.USE_ACCOUNT,
						acctHash, inputMaxLen, new byte[] { 'F', 'F', 'F', 'F',
								'F', 'F', 'F', 'F', 'F', 'F' }, true, msg, 30,
						TimeUnit.SECONDS);
		return event;
