...
public class RKDFaceCollectSucUtil {
    ...
    private RYDBossBaseInfoActivity baseInfoSuccesActivity;


    private static RKDFaceCollectSucUtil instance;

    public static RKDFaceCollectSucUtil getInstance() {
        if (instance == null) {
            instance = new RKDFaceCollectSucUtil();
        }
        return instance;
    }
    ...
    public void initSucPage(Context context, Bundle result) {
        ...
        mcontext = context;
        baseInfoSuccesActivity = (RYDBossBaseInfoActivity) context;
        ...
        if(result.containsKey("credit_type"))
            credit_type = result.getString("credit_type");
        initView();
                ...
                ContactsResponse.class, new ICallback<ContactsResponse>() {
                    ...
                    public void success(ContactsResponse contactsResponse) {
                        ...
                        if (contactsCode==5031) {
                            baseInfoSuccesActivity.showMaintainDialog();
                        } else {
                            (baseInfoSuccesActivity).setResult(BORROW_RESULT_CODE);
                            (baseInfoSuccesActivity).finish();
                        }
    ...
    private void doActive(String face_id, String score) {
        ...
        request.setCredit_type(credit_type);
        request.setFlag("A");
        request.setKind("B");
        baseInfoSuccesActivity.httpsPost(baseInfoSuccesActivity, request, ReqAction.CASH_LIMIT, CActivateLimitResponse.class, new ICallback<CActivateLimitResponse>() {
            ...
            public void success(CActivateLimitResponse cActivateLimitResponse) {
                int cActivateLimitCode = cActivateLimitResponse.getCode();
                if (cActivateLimitCode==5031) {
                    baseInfoSuccesActivity.showMaintainDialog();
                } else {
