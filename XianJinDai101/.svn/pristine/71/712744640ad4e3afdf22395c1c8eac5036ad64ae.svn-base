...
public class CashFragment extends BaseFragment {
    ...
    public LinkedList<TradeDetailInfo> cashList = new LinkedList<TradeDetailInfo>();
    ...
    public static CashFragment newInstance() {
        CashFragment fragment = new CashFragment_();
        return fragment;
    }
    ...
    private void initViews() {
        ...
        lv_cash.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intent = new Intent((BMIncomeAndExpenditureDetails2)mListener,
                        DetailsTabMainActivity_.class);
                intent.putExtra("detailInfo", cashList.get(position));
                intent.putExtra("ActivityFlag", "income");
                intent.putExtra("fromtype", 1); // 1 代表是 查看明细
                startActivity(intent);
            }
        });
        adapter = new IncomeAdapter((BMIncomeAndExpenditureDetails2) mListener, cashList);
