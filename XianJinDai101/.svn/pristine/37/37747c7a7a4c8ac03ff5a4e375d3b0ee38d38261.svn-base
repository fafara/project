 ...
 * 确认借款
 */
public class RYDSureBorrowActivity extends BaseActivity implements View.OnClickListener {
...
//    private TextView daozhagnAmountTv;
    ...
    private String borrowAmountTv_double;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.c_activity_ryd_sure_borrow);
        setTitleLayout(getResources().getString(R.string.c_sure_borrow), true, false);
        initView();
        borrowSureTip1.setText(Html.fromHtml(getResources().getString(R.string.c_sure_borrow_tipryd)));
    ...
    private void getRoute() {
        ...
        httpsPost(this, request, ReqAction.APPLICATION_ROUTE, CpamentRouteResponse.class, new ICallback<CpamentRouteResponse>() {
            ...
            public void failture(String tips) {
                CLogUtil.showToast(RYDSureBorrowActivity.this, tips);
...
//        service_agreement = bundle.getString("service_agreement_url");
        ...
        sub_cost_rate = bundle.getString("sub_cost_rate");
        ...
        agreement_id = bundle.getInt("agreement_id");
        card_type = bundle.getInt("card_type");
        cost_rate = bundle.getDouble("cost_rate");
        //待还金额
        term_amount = bundle.getDouble("term_amount");
    }
    ...
    private void initView() {
        ...
        term_amount_double =df.format(term_amount);
        pendingAmountTv.setText("¥" + term_amount_double);//待还金额
...
//        tv_borrowterm = (TextView) findViewById(tv_borrowterm);
        ...
        sureBorrowBtn.setOnClickListener(new NoDoubleClickListener() {
            ...
            protected void onNoDoubleClick(View view) {
                if ("1".equals(chanllenge_type)) {
                    ...
                    Intent intent = new Intent(RYDSureBorrowActivity.this, FaceCollectActivity.class);
                    intent.putExtra("applyRequest", (Serializable) applyRequest);
                    startActivityForResult(intent, IDENTIFY_RECORDS_CODE);
                }
            }
        });
        CBanksUtils.selectIcoidToImgView(RYDSureBorrowActivity.this, dfTbankCode, shoukuanAmountIv);
        CBanksUtils.selectIcoidToImgView(RYDSureBorrowActivity.this, dkTbankCode, huankuanAmountIv);
    }

    public void showBottomSheet() {
        final BottomSheetDialog mBottomSheetDialog = new BottomSheetDialog(RYDSureBorrowActivity.this, R.style.Material_App_BottomSheetDialog);
        View boottomView = LayoutInflater.from(RYDSureBorrowActivity.this).inflate(R.layout.c_popwindow_sms_check, null);
        ...
        buttonSure.setOnClickListener(new NoDoubleClickListener() {
            ...
            protected void onNoDoubleClick(View view) {
                if (TextUtils.isEmpty(etPhonecode.getText().toString().trim())) {
                    CLogUtil.showToast(RYDSureBorrowActivity.this, "请填写验证码!");
                    return;
                }
                applyRequest.setCode(etPhonecode.getText().toString().trim());
                applyRequest.setAgreement_id(agreement_id);
                httpsPost(RYDSureBorrowActivity.this, applyRequest, ReqAction.APPLICATION_LOAN_APPLY, CLoanApplyResponse.class, new ICallback<CLoanApplyResponse>() {
                    @Override
                    public void success(CLoanApplyResponse cLoanApplyResponse) {
                        mBottomSheetDialog.dismiss();
                        int routeCode = cLoanApplyResponse.getCode();
                        if (routeCode==5031) {
                            showMaintainDialog();
                        } else {
                            //如果返回的申请编码不为空，则表示借款成功
                            if (!TextUtils.isEmpty(cLoanApplyResponse.getResult())) {
                                CCommonDialog.showRepaymentOK(RYDSureBorrowActivity.this,
                                        "借款申请成功", "请随时关注银行卡到账信息！",
                                ...
                                CLogUtil.showToast(RYDSureBorrowActivity.this, "对不起，您的综合平分不足！");
                                setResult();
                            }
                        }
                    }
        ...
        httpsPost(this, request, ReqAction.APPLICATION_MESSAGE_CHALLENGECODE_SEND, CMsgSendResponse.class, new ICallback<CMsgSendResponse>() {
            ...
            public void success(CMsgSendResponse cMsgSendResponse) {
                ...
                if (cMsgSendCode==5031) {
                    ...
                    CLogUtil.showToast(RYDSureBorrowActivity.this, "验证码已发送!");
                }
            }
    ...
    private String appendManagerUrl(String url) {
        ...
        try {
            finalUrl = url + "?name=" + URLEncoder.encode(RyxcreditConfig.getRealName(),
                    ...
                    /*subZeroAndDot(CNummberUtil.getStrFromBigDecimal(cost_rate).multiply(b2).toString())*/
                    + "&service_amount=" + other_cost_amount
                    + "&breach_contract_fee=" +  df.format(CNummberUtil.getStrFromBigDecimal(String.valueOf(other_overdue_interest_rate)));
    ...
    private String appendContractUrl(String url) {
        ...
        try {
            ...
            finalUrl = url + "?name=" + URLEncoder.encode(RyxcreditConfig.getRealName(), "UTF-8")
                    ...
                    //借款利息
                    +"&brinterest= "+df.format(CNummberUtil.getStrFromBigDecimal(sub_cost_amount))
                   // + "&brinterest=" + subZeroAndDot(CNummberUtil.getStrFromBigDecimal(sub_cost_rate).multiply(b2).toString())
                    //逾期罚息
                    +"&overdue_rate="+df.format(overdue_interest_rate)
                    //借款利率
                    +"&brinterestrate= "+df.format(CNummberUtil.getStrFromBigDecimal(sub_cost_rate).multiply(b2));
