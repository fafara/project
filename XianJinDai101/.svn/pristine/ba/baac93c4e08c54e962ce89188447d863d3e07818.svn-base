...
@EActivity(R.layout.activity_jdrecharge)
public class JDRechargeActivity extends BaseActivity {
    ...
    @ViewById(R.id.gv_jd)
    GridView mJDGridView;
    private String mMoney;
    private Param qtpayGameId;
    private String gameid = "CARD60261";
    ...
    public void initViews() {
        setTitleLayout("京东卡充值", true);
        setRightImgMessage("用户须知", RyxAppconfig.Notes_JDRecharge);
        initQtPatParams();
        doOrderRequest();
    }
    ...
    private void doOrderRequest() {
        ...
        httpsPost("doAliPayReq", new XmlCallback() {
            ...
            public void onTradeSuccess(RyxPayResult payResult) {
                ...
                LogUtil.showLog("muxin", "alipay json:" + jsonStr);
                initListData();
                initData();
            }
        });
    ...
    private void initData() {
        mMobileEt.setText(QtpayAppData.getInstance(JDRechargeActivity.this).getMobileNo());
        mMobileEt.setSelection(mMobileEt.getText().length());
        adapter = MobileAdapter_.getInstance_(JDRechargeActivity.this);
        adapter.setList(coinlist);
        adapter.setSelection(0);
        mMoney = MoneyEncoder.EncodeFormat(coinlist.get(0).getParvalue());
        mJDGridView.setAdapter(adapter);
    }

    @ItemClick(R.id.gv_jd)
    ...
    private boolean checkInput() {
        ...
        if (TextUtils.isEmpty(mobile) || mobile.length() != 11) {
            LogUtil.showToast(JDRechargeActivity.this, getResources().getString(R.string.please_enter_the_11_phone_numbers));
            return false;
        } else if (mobile.substring(0, 1).equals("0")) {
            LogUtil.showToast(JDRechargeActivity.this, getResources().getString(R.string.not_a_phone_number));
            return false;
        } else if (TextUtils.isEmpty(mMoney)) {
            LogUtil.showToast(
                    JDRechargeActivity.this, "请选择充值金额!!!");
            return false;
        }
        return true;
    }
    ...
    public void rechargeClick(){
        if (checkInput()) {
            Intent intent = new Intent(JDRechargeActivity.this, CreateOrder_.class);
            orderinfo = Order.JD_RECHARGE;
            ...
            Log.i("muxin", "desc:"+ orderinfo.getOrderDesc());
            orderinfo.setOrderAmt(mMoney);
            LogUtil.showLog("muxin", "money:"+mMoney);
            orderinfo.setOrderRemark(mobile);
            LogUtil.showLog("muxin", "qqnum:"+mobile);
