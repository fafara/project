 ...
 * 光宇币
 */
@EActivity(R.layout.activity_gy_pay)
public class GYActivity extends BaseActivity {

    @ViewById(R.id.gv_gypay)
    GridView mGYPayGridView;
    ...
    private String gameid = "GAME1066";
    ...
    public void initViews() {
        setTitleLayout("光宇币", true);
        setRightImgMessage("用户须知", RyxAppconfig.Notes_GYRecharge);
        initQtPatParams();
        doOrderRequest();
    }
    ...
    private boolean checkInput() {
        mobile = mGYPayEt.getText().toString().trim();
        if (TextUtils.isEmpty(mobile)) {
            LogUtil.showToast(GYActivity.this, getResources().getString(R.string.please_enter_account));
            return false;
        }else if (TextUtils.isEmpty(mMoney)) {
            LogUtil.showToast(
                    GYActivity.this, "请选择充值金额!!!");
            return false;
        }
        return true;
    }
    ...
    public void rechargeClick() {
        if (checkInput()) {
            Intent intent = new Intent(GYActivity.this, CreateOrder_.class);
            orderinfo = Order.GY_RECHARGE;
    ...
    private void initData() {
        adapter = QqCoinAccountAdapter_.getInstance_(GYActivity.this);
        adapter.setList(coinlist);
        adapter.setSelection(0);
        mMoney = MoneyEncoder.EncodeFormat(coinlist.get(0).getParvalue());
        mGYPayGridView.setAdapter(adapter);
    }
