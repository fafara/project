package com.ryx.ryxgesturespswd.util;
...
public class LockPatternUtil {
     ...
     * @return Whether the pattern matches the stored one.
     */
    public static boolean checkPaswd(List<LockPatternView.Cell> pattern, String paswd) {
        if (pattern == null || TextUtils.isEmpty(paswd)) {
            return false;
        } else {
            byte[] bytes2 = patternToHash(pattern);
//            return Arrays.equals(bytes, bytes2);
            return paswd.equals(Arrays.toString(bytes2));
        }
    }
}
