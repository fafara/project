 ...
 * 波克城市
 */
@EActivity(R.layout.activity_bk_pay)
public class BoKeActivity extends BaseActivity {

    @ViewById(R.id.gv_bkpay)
    GridView mBKPayGridView;
    ...
    public void initViews() {
        setTitleLayout("波克城市", true);
        setRightImgMessage("用户须知", RyxAppconfig.Notes_BKRecharge);
        initQtPatParams();
        doOrderRequest();
    }
    ...
    private boolean checkInput() {
        mobile = mBKPayEt.getText().toString().trim();
        if (TextUtils.isEmpty(mobile)) {
            LogUtil.showToast(BoKeActivity.this, getResources().getString(R.string.please_enter_account));
            return false;
        } else if (TextUtils.isEmpty(mMoney)) {
            LogUtil.showToast(
                    BoKeActivity.this, "请选择充值金额!!!");
            return false;
        }
        return true;
    }
    ...
    public void rechargeClick() {
        if (checkInput()) {
            Intent intent = new Intent(BoKeActivity.this, CreateOrder_.class);
            orderinfo = Order.BK_RECHARGE;
    ...
    private void initData() {
        adapter = QqCoinAccountAdapter_.getInstance_(BoKeActivity.this);
        adapter.setList(coinlist);
        adapter.setSelection(0);
        mMoney = MoneyEncoder.EncodeFormat(coinlist.get(0).getParvalue());
        mBKPayGridView.setAdapter(adapter);
    }
    ...
    private void initListData() {
        try {
            ...
            for (int i = 0; i < banks.length(); i++) {
                ...
                qcoinInfo.setTipName("波克城市" + parvalue);
